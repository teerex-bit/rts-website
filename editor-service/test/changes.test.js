...
test('preview does not write; approved change writes one override',async()=>{
 ...
 const r=await f.editor.apply(p.changeToken,'320403715',change);assert.equal(r.commitSha,'b'.repeat(40));assert.equal(f.writes(),1);
 await assert.rejects(()=>f.editor.apply(p.changeToken,'320403715',change),/STALE/);assert.equal(f.writes(),1);
});
test('approval must repeat the human-readable previewed change',async()=>{
 const f=fixture();const p=await f.editor.preview(change,'320403715');
 await assert.rejects(()=>f.editor.apply(p.changeToken,'320403715',{...change,value:'Different heading'}),/MISMATCH/);
...
test('unauthorized users, locked pages, forged and stale changes are rejected',async()=>{
 ...
 await assert.rejects(()=>f.editor.apply(p.changeToken+'x','320403715',change),/INVALID/);
 f.stale();await assert.rejects(()=>f.editor.apply(p.changeToken,'320403715',change),/STALE/);assert.equal(f.writes(),0);
});
test('expired preview tokens are rejected',async()=>{
 ...
 await assert.rejects(()=>late.apply(p.changeToken,'320403715',change),/EXPIRED/);
});
