...
for(const p of pages){const dir=path.join(out,p.route);fs.mkdirSync(dir,{recursive:true});const html=p.number===0?fs.readFileSync(path.join(__dirname,'..','src','page-00-approved.html'),'utf8'):p.number===1?fs.readFileSync(path.join(__dirname,'..','src','page-01-approved.html'),'utf8').replace('/assets/styles.css','/assets/page-01-approved.css'):p.number===38?fs.readFileSync(path.join(__dirname,'..','src','conversations-approved.html'),'utf8').replace('/assets/styles.css','/assets/conversations-approved.css'):shell(p.title,main(p));fs.writeFileSync(path.join(dir,'index.html'),html)}
const review=`${header()}<main class="review"><p class="overline">Review site</p><h1>All 41 pages</h1><p class="lead">A complete index of the Reforming the Soul journey.</p><ol>${pages.map(p=>`<li><span>${String(p.number).padStart(2,'0')}</span><a href="${p.route}">${esc(p.title)}</a><small>${p.stage} · ${p.template}</small></li>`).join('')}</ol></main>${footer()}`;
