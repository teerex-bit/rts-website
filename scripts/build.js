...
const conversationsHeader=()=>`<header class="site-header"><a class="brand" href="${links.home}" aria-label="Reforming the Soul home"><img src="/assets/logo-circle.svg" alt="Reforming the Soul"></a><button type="button" class="menu" aria-expanded="false" aria-controls="main-nav">Menu</button><nav id="main-nav" aria-label="Main navigation"><a href="${links.awaken}">Awaken</a><a href="${links.seeClearly}">See Clearly</a><a href="${links.become}">Become</a><a href="${links.join}">Join</a><a class="is-active" href="${links.conversations}">Conversations</a></nav></header>`;
