...
for(const p of pages){const dir=path.join(out,p.route);fs.mkdirSync(dir,{recursive:true});const html=p.number===1?fs.readFileSync(path.join(__dirname,'..','src','page-01-approved.html'),'utf8').replace('/assets/styles.css','/assets/page-01-approved.css'):shell(p.title,main(p));fs.writeFileSync(path.join(dir,'index.html'),html)}
