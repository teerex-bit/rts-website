...
if(!fs.existsSync(page00Path))errors.push(`${page00Path}: missing Soul Formation route`);
  ...
  if((resources.match(/class="page00-resource\s/g)||[]).length!==2)errors.push(`${page00Path}: expected two supporting resource cards`);
  for(const label of ['Conversations','Music'])if(!resources.includes(`<h3>${label}</h3>`))errors.push(`${page00Path}: missing ${label} resource card`);
  if(/Writings|Blogs|<h3>Soul Formation<\/h3>|<h3>Books<\/h3>/i.test(resources))errors.push(`${page00Path}: supporting resources must be Conversations and Music only`);
}
...
if(/course-top:has\([^}]+\):before\s*\{[^}]*curriculum-logo-transparent\.png/.test(builtStyles))errors.push(`${builtStylesPath}: Pages 03-06 must not render a duplicate pseudo-element curriculum logo`);
