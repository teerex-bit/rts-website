const { journey } = require('./shared');

module.exports = {
  number: 13,
  route: '/see-god-clearly/',
  title: 'Seeing God Clearly',
  family: 'landing',
  course: 'See Clearly',
  lesson: '',
  progress: 'See Clearly',
  progressValue: 2,
  progressMax: 2,
  step: 2,
  journey,
  eyebrow: 'See Clearly',
  subheading: 'The Next Essential Step',
  introduction: [
    'You’ve learned to see yourself more clearly.',
    'But all the self-examination in the world will not solve a single thing if we have a misunderstanding of who God is.',
    'Your view of God shapes everything—how you relate, how you trust, how you grow, and how you live.'
  ],
  invitation: 'It’s time to discover who He truly is.',
  heroImage: '/assets/page-ranges/range-11-15/seeing-god-forest.jpg',
  movements: [
    {
      number: '01',
      part: 'Part 1',
      title: 'See Yourself Clearly',
      text: 'Notice the stories, assumptions, and patterns that have shaped the way you see.'
    },
    {
      number: '02',
      part: 'Part 2',
      title: 'See God Clearly',
      text: 'Look again through Jesus and receive the truer reality of God’s love and goodness.'
    }
  ],
  selectedMovement: 1,
  discoveries: ['Who God is beyond the misconceptions and distortions we’ve picked up.', 'How His character reveals His heart toward you.', 'Why knowing God rightly changes everything.', 'How to relate to God as He truly is—Father, not taskmaster.'],
  scripture: ['This is eternal life:', 'that they know You,', 'the only true God,', 'and Jesus Christ', 'whom You have sent.'],
  citation: 'John 17:3',
  matters: [
    ['It’s Not About Performance', 'God is not waiting for you to get better before He loves you.'],
    ['Frees You From Fear and Guilt', 'When you know God, you can trust Him.'],
    ['Unlocks True Transformation', 'Change happens in the safety of God’s love, not under pressure.'],
    ['Empowers You to Live with Him', 'Right relationship leads to right living.']
  ]
};
