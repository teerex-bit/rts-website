...
function renderLanding(page) {
  const movements = page.movements ? `<section class="rts-11-15__landing-parts" aria-labelledby="page-13-movements-title"><div class="rts-11-15__landing-parts-heading"><span>${icon('eye')}</span><p id="page-13-movements-title">Two Movements. One Journey.</p></div><div class="rts-11-15__landing-parts-steps">${page.movements.map((movement, index) => `<article class="${index === 0 ? 'is-active' : ''}"><span>${esc(movement.number)}</span><div><small>${esc(movement.part)}</small><h3>${esc(movement.title)}</h3><p>${esc(movement.text)}</p></div></article>${index === 0 ? '<i aria-hidden="true">→</i>' : ''}`).join('')}</div></section>` : '';
