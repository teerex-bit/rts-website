...
function rail(active, detailed = false, progress = '') {
  const stages = [
  ...
  const progressPanel = progress ? `<section class="r1620-rail__progress"><small>Your Progress</small><span>${escapeHtml(progress)}</span><i><b style="width:83.333%"></b></i></section>` : '';
...
function renderSeeClearly(page) {
  ...
  return `<main class="r1620 r1620--reflection" data-page-number="${page.number}" data-editable-source="range-16-20">
    ${rail(page.stage, false, page.progress)}
...
function renderLanding(page) {
  return `<main class="r1620 r1620--landing" data-page-number="${page.number}" data-editable-source="range-16-20">
    ...
    <section class="r1620-landing-hero">
      <img src="${assetRoot}/becoming-hero.webp" alt="Woman overlooking a mountain valley at sunrise">
...
function renderLessonOne(page) {
  const content = `<section class="r1620-lesson-hero">
      <img src="${assetRoot}/presence-path.webp" alt="Sunrise over a mountain trail">
      <div><p class="r1620-eyebrow">${page.course}</p><h1>${page.title}</h1><div class="r1620-gold-rule"></div><h2>${page.headline}</h2><p>${page.intro}</p><div class="r1620-actions"><a href="#lesson-content">Begin Lesson →</a><a href="/become/">▷ &nbsp; Watch Overview</a></div></div>
...
function renderLessonTwo(page) {
  const content = `<section class="r1620-pattern-hero"><img src="${assetRoot}/presence-path.webp" alt="Sunrise over a mountain trail"><div><p class="r1620-eyebrow">${page.course}</p><p class="r1620-screen-label">${page.screen}</p><h1>${page.title}</h1><div class="r1620-gold-rule"></div><h2>${page.headline}</h2><p>${page.intro}</p></div>
